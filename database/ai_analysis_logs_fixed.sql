...
-- Composite index for cost analysis (fixed - removed ::date cast)
CREATE INDEX IF NOT EXISTS idx_ai_analysis_logs_cost_tracking ON ai_analysis_logs(model_provider, model_name, created_at, estimated_cost);

-- View for daily cost analysis with token breakdown
...
-- Add index for token columns (for efficient queries on token usage)
